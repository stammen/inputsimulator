...
private:
    ...
    ValueSet^ HandleMessage(ValueSet^ message)
    {
        ...
        else if (data->HasKey(L"KEYBDINPUT"))
        {
            INPUT input;
            ...
            {
                response->Insert(L"Status", "Error");
                response->Insert(L"StatusMessage", "Error sending KEYBDINPUT with SendInput()");
            }
        }
		else if (data->HasKey(L"HARDWAREINPUT"))
		{
			INPUT input;
			input.type = INPUT_HARDWARE;

			input.hi.uMsg = static_cast<unsigned int>(data->Lookup(L"uMsg"));
			input.hi.wParamL = static_cast<int>(data->Lookup(L"wParamL"));
			input.hi.wParamH = static_cast<int>(data->Lookup(L"wParamH"));
			...
			{
				response->Insert(L"Status", "Error");
				response->Insert(L"StatusMessage", "Error sending HARDWAREINPUT with SendInput()");
			}
		}
        else
        {
